...
df <- read.csv("data/data.csv") |>
  mutate(
    ...
    Stimulus_Interest = case_when(
      ...
      Sexual_Orientation == "Bisexual" ~ TRUE,
      TRUE ~ NA
    )
  )
...
df <- df |>
  ...
  summarise(Confidence = mean(abs(Belief_Confidence)),
            n = n() / 109) |> 
...
for (participant in dfsub$Participant) {
  ...
  dfsub[dfsub$Participant == participant, "r_Goodlooking"] <- cor(df[df$Participant == participant, "Goodlooking"], df[df$Participant == participant, "Norms_Attractive"])
}
```
...
p5 <- plot_waffle(dfsub, "Nationality") +
...
patchwork::wrap_plots(list(p1, p2, p3, p4, p5, p6, p7, p8))
```
...
```{r message=FALSE, warning=FALSE}
IVs <- c("Attractive", "Goodlooking", "Trustworthy", "Familiar")
...
for (x in IVs) {
  for (y in IVs) {
    ...
    model <- glmmTMB::glmmTMB(as.formula(
      paste(y, "~", x, "* Sex * Stimulus_SameSex + (1|Participant) + (1|Stimulus)")
    ),
    data = df,
    ...
    pred <- estimate_relation(model, at = c(x, "Stimulus_SameSex", "Sex"), length = 20)
    pred$y <- y
...
dats |>
  ...
  geom_line(data = preds, aes(color = Sex, linetype=Stimulus_SameSex)) +
...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB::glmmTMB(Belief ~ poly(Delay, 2) + (1|Participant) + (1|Stimulus),
      data = df,
      family = "binomial"
    )
pred <- estimate_relation(model, at = "Delay", length = 20) 

m_conf <- glmmTMB::glmmTMB(Belief_Confidence ~ Belief / poly(Delay, 2) + (1|Participant) + (1|Stimulus),
      data = df,
      family = glmmTMB::beta_family()
    )
...
make_model <- function(df, var = "Attractive", formula=var, fill = "#2196F3") {
  m_real <- glmmTMB::glmmTMB(as.formula(paste0("Belief ~ ", formula)),
                             data = df,
                             family = "binomial")
  y_real <- estimate_relation(m_real, at = c(var, "Sex"), length = 20)
  ...
  y_conf <- estimate_relation(m_conf, at = c(var, "Belief", "Sex"), length = 20)
  y_conf <- y_conf |> 
    mutate_at(c("Predicted", "CI_low", "CI_high"), function(x) ifelse(y_conf$Belief == "Real", datawizard::rescale(x, range=c(0, 1), to=c(0.5, 1)), datawizard::rescale(x, range=c(1, 0), to=c(0, 0.5))))
  ...
  p <- df |>
    ...
    geom_ribbon(data = y_real, aes(y = Predicted, group=Sex, fill=Sex, ymin = CI_low, ymax = CI_high), alpha = 0.5) +
    geom_line(data = y_real, aes(y = Predicted, color=Sex)) +
  ...
  list(p=p, model_belief=m_real)
}
```

```{r message=FALSE, warning=FALSE}
rez_at <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "Sex / poly(Attractive, 2) + Trustworthy + (1|Participant) + (1|Stimulus)", 
                     var="Attractive", fill = "#F44336")
rez_gl <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "Sex / poly(Goodlooking, 2) + Trustworthy + (1|Participant) + (1|Stimulus)", 
                     var="Goodlooking", fill = "#E91E63")
rez_tr <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "Sex / poly(Trustworthy, 2) + Goodlooking + (1|Participant) + (1|Stimulus)", 
...
parameters::parameters(rez_at$model_belief)
parameters::parameters(rez_gl$model_belief)
parameters::parameters(rez_tr$model_belief)
parameters::parameters(rez_fa$model_belief)
...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB::glmmTMB(Belief ~ 1 + (1|Participant) + (1|Stimulus), data=df, family="binomial")
dfsub <- modelbased::estimate_grouplevel(model) |> 
  filter(Group == "Participant") |> 
  transmute(Participant = Level, Simulation_Monitoring=Coefficient) |> 
  merge(dfsub, by="Participant")

model <- glmmTMB::glmmTMB(Belief_Confidence ~ Belief + (Belief|Participant) + (1|Stimulus), data=df, family=glmmTMB::beta_family())
dfsub <- modelbased::estimate_grouplevel(model) |> 
  filter(Group == "Participant") |> 
  transmute(Participant = Level, Parameter, Coefficient) |> 
  pivot_wider(names_from = "Parameter", values_from = "Coefficient") |> 
  transmute(Participant, Confidence_Real_Model = BeliefReal, Confidence_Fake_Model = `(Intercept)`) |> 
  merge(dfsub, by="Participant")
```
```{r message=FALSE, warning=FALSE}
correlation::correlation(dfsub[c("Simulation_Monitoring", "Confidence_Fake_Model", "Confidence_Real_Model")], 
                         dfsub[c("Confidence_Fake", "Confidence_Real", "n_Real")],
                         )
```
...
sr <- c("Simulation_Monitoring", "Confidence_Fake_Model", "Confidence_Real_Model", "Confidence_Fake", "Confidence_Real", "n_Real")
