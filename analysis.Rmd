...
df |>
  ...
  geom_line(data=y_conf, aes(y=Predicted, group=Belief), linetype = "dashed") +
...
make_model <- function(df, var = "Attractive", formula=var, fill = "#2196F3") {
  ...
  p <- df |>
    ...
    theme_modern(axis.title.space=0) + 
...
rez_at <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "poly(Attractive, 2) * Sex + Trustworthy + (1|Participant) + (1|Stimulus)", 
                     var="Attractive", fill = "#F44336")
rez_gl <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "poly(Goodlooking, 2) * Sex + Trustworthy + (1|Participant) + (1|Stimulus)", 
                     var="Goodlooking", fill = "#E91E63")
rez_tr <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "poly(Trustworthy, 2) * Sex + Goodlooking + (1|Participant) + (1|Stimulus)", 
                     var="Trustworthy", fill = "#4CAF50")
rez_fa <- make_model(filter(df, Stimulus_Interest == TRUE), 
                     formula = "poly(Familiar, 2) * Sex + (1|Participant) + (1|Stimulus)", 
